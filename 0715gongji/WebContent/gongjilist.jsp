...
<%
	...
	String QueryTxt;
	QueryTxt = String.format("select id, relevel, title, date, viewcnt from gongji order by rootid DESC, relevel ASC, recnt ASC;");
...
<div>
	<div align=center><h1>공지사항 리스트</h1></div>
		<table class="parent" cellspacing=1 width=700 border=1 align=center>
			<tr>
...
<%
			while(rset.next()){
				...
				out.println("<a href=gongji_view.jsp?key="+rset.getInt(1)+">"+rset.getString(3)+"</a></td>");
...
%>				
		...
		<table border="0" align="center">
			...
			<%if(firstLinkNum != 1){%>
			<td align="center">
			<a href="gongjilist.jsp?sendFirstLink=<%=firstPage_firstLinkNum %>">≪</a>
			</td>
			<td align="center">
			<a href="gongjilist.jsp?sendFirstLink=<%=prePage_firstLinkNum %>&from=<%=prePage_firstLinkNum*10-9 %>&cnt=10">＜</a>
			</td>
			<%}%>
<%
			for(int i=firstLinkNum; i<firstLinkNum+10; i++){
				...
				out.print("<td>");   
                out.print("<a href=gongjilist.jsp?sendFirstLink="+firstLinkNum+"&from="+((i*10)-9)+"&cnt=10>"+i+"</a>");
                out.print("</td>");
			}	
%>
			<%if((firstLinkNum+9)*10 < totalLineCnt){%>
				<td> 
				<a href="gongjilist.jsp?sendFirstLink=<%=nextPage_firstLinkNum%>&from=<%=nextPage_firstLinkNum*10-9 %>&cnt=10">
				＞</a>
				</td>
				<td> 
				<a href="gongjilist.jsp?sendFirstLink=<%=lastPage_firstLinkNum%>&from=<%=lastPage_firstLinkNum*10-9 %>&cnt=10">
				≫</a>
				</td>
		...
		<tr align=right>
		<td><button type="button" onclick="location.href='gongji_insert.jsp'">신규</button></td>
		</tr>
		</table>
